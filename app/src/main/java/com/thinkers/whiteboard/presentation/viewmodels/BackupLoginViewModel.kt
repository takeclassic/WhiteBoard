...
@HiltViewModel
class BackupLoginViewModel @Inject constructor(private val createAccountUseCase: CreateAccountUseCase,
    ...
    suspend fun getAuthResult(authType: AuthType): AuthInfo<AuthResult> = suspendCancellableCoroutine { cont ->
        ...
        when(authType) {
            AuthType.LOGIN -> {
                signInUseCase(auth!!, id, password, listener)
            }
            AuthType.REGISTER -> {
                createAccountUseCase(auth!!, id, password, listener)
            }
        }
    }
